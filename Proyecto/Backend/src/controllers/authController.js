...
export const googleAuth = async (request, response) => {
  ...
  if (!usuario) {
    ...
    usuario = await prisma.usuario.create({
      data: {nombre: name, correo: email, googleId: googleId, contraseña: ''},
    });

    sendWelcomeEmail(email, name);

  } else if (!user.googleId) user = await prisma.usuario.update({where: { id: user.id }, data: { googleId: googleId },});

  const token = jwt.sign({ id_usuario: user.id }, process.env.JWT_SECRET, { expiresIn: "3650d" });

  return response.json({ ok: true, userId: user.id, correo: user.correo, token, keepLogged: true });
};
