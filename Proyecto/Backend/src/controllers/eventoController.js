...
export const crearEvento = async (req, res) => {
  ...
  const nuevoEvento = await prisma.evento.create({
    data: { id_mascota, titulo, descripcion, fecha_inicio: new Date(fecha_inicio),
...
export const obtenerEventos = async (req, res) => {
  ...
  const eventosMascota = await prisma.evento.findMany({where: { mascota: { id_usuario: userId}},
...
export const editarEvento = async (req, res) => {
  ...
  const eventoActualizado = await prisma.evento.update({ where: { id: parseInt(id) },
    data: {
      ...(titulo && { titulo }),
