export function validateEmailFields(email: string) {
  if (!email || email.trim() === "") return {valido: false, error: 'ⓘ Por favor ingrese un correo'};
  ...
  if (local.startsWith(".") || local.endsWith(".")) return {valido: false, error: 'ⓘ El formato no es válido'};
  if (domain.startsWith(".") || domain.endsWith(".")) return {valido: false, error: 'ⓘ El formato no es válido'};
