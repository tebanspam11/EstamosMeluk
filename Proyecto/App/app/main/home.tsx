...
export default function HomeScreen({ navigation }: any) {
  ...
  const cargarDatos = async () => {
    ...
    const [usuarioRes, mascotasRes, eventosRes] = await Promise.all([fetch(`${API_URL}/usuarios`, { headers }), fetch(`${API_URL}/mascotas`, { headers }), fetch(`${API_URL}/eventos`, { headers })]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.remindersSection}>
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.headerButtons}>
            <TouchableOpacity style={styles.addEventButton} onPress={() => navigation.navigate('Calendar')}>
        ...
        {eventos.length === 0 ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.remindersScroll}>
            {eventos.map((evento) => (
              <TouchableOpacity key={evento.id} style={styles.reminderCard} onPress={() => navigation.navigate('Calendar')}>
      ...
      <View style={styles.petsSection}>
        ...
        {mascotas.length === 0 ? (
          <View style={styles.emptyCard}>
            <Text style={styles.emptyIcon}>🐾</Text>
            <Text style={styles.emptyText}>Nada por aquí...</Text>
            <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('PetRegister')}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.petsScroll}>
            {mascotas.map((mascota) => (
              <TouchableOpacity key={mascota.id} style={styles.petCard} onPress={() => navigation.navigate('PetProfile', { pet: mascota })}>
                {mascota.foto ? (
                  <Image source={{ uri: mascota.foto }} style={styles.petImage} />
                ) : (
                  <Text style={styles.petIcon}>{mascota.especie.toLowerCase() === 'perro' ? '🐶' : mascota.especie.toLowerCase() === 'gato' ? '🐱' : '🐾'}</Text>
                )}
            ...
            <TouchableOpacity style={[styles.petCard, styles.addPetCard]} onPress={() => navigation.navigate('PetRegister')}>
      ...
      <View style={styles.bottomNav}>
        ...
        <TouchableOpacity style={styles.navButton} onPress={() => navigation.navigate('VeterinarySearch')}>
          <Text style={styles.navIcon}>🏥</Text>
          <Text style={styles.navText}>Veterinarias</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.navButton} onPress={() => navigation.navigate('Carnet')}>
          <Text style={styles.navIcon}>📄</Text>
          <Text style={styles.navText}>Carnet</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.navButton} onPress={() => navigation.navigate('ClinicHistory')}>
          <Text style={styles.navIcon}>🏥</Text>
