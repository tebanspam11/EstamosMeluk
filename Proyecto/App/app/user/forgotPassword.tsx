...
export default function ForgotPasswordScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    setPasswordValid(newPassword ? validatePasswordChecklist(newPassword).valido : false);
  }, [newPassword]);

  useEffect(() => {
    if (confirmPassword && newPassword) {
      setConfirmPasswordError(validateConfirmPasswordFields(newPassword, confirmPassword).valido ? null : validateConfirmPasswordFields(newPassword, confirmPassword).error);
    } else {
  ...
  const handleResetPassword = async () => {
    ...
    if (response.ok && data.ok) {
      Alert.alert('¡Contraseña actualizada!', 'Tu contraseña ha sido cambiada exitosamente. Ahora puedes iniciar sesión con tu nueva contraseña.', [
        {
          text: 'Ir al Login',
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {step === 1 && (
          <View style={styles.form}>
            ...
            <TextInput
              ...
              autoCapitalize="none"
            />
            <TouchableOpacity style={[styles.button, (loading || !email.trim()) && styles.buttonDisabled]} onPress={handleSendCode} disabled={loading || !email.trim()}>
        ...
        {step === 2 && (
          <View style={styles.form}>
            <Text style={styles.instructions}>Hemos enviado un código de 6 dígitos a {email}</Text>
            <Text style={styles.label}>Código de 6 dígitos</Text>
            <TextInput style={styles.input} placeholder="123456" placeholderTextColor="#999" keyboardType="number-pad" maxLength={6} value={code} onChangeText={setCode} />
            <TouchableOpacity style={[styles.button, (loading || code.length !== 6) && styles.buttonDisabled]} onPress={handleVerifyCode} disabled={loading || code.length !== 6}>
