...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator, TextInput, Modal, Platform, Linking } from 'react-native';
...
export default function ClinicHistoryScreen({ navigation }: any) {
  ...
  const viewDocument = async (documento: Documento) => {
    try {
      ...
      const url = documento.archivo_pdf && documento.archivo_pdf.startsWith('http') ? documento.archivo_pdf : `${baseNoApi}${rel}`;
  ...
  const deleteDocument = async (id: number) => {
    Alert.alert('Eliminar Documento', '¿Estás seguro de que deseas eliminar este documento?', [
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Selector de Mascota */}
      {mascotas.length > 0 && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.petSelector} contentContainerStyle={styles.petSelectorContent}>
          {mascotas.map((mascota) => (
            <TouchableOpacity key={mascota.id} style={[styles.petOption, selectedMascota?.id === mascota.id && styles.selectedPetOption]} onPress={() => setSelectedMascota(mascota)}>
              <Text style={styles.petIcon}>{mascota.especie.toLowerCase() === 'perro' ? '🐶' : '🐱'}</Text>
        ...
        {documentos.length === 0 ? (
          ...
          documentos.map((doc) => (
            <View key={doc.id} style={styles.documentCard}>
              ...
              {doc.descripcion && <Text style={styles.documentDescription}>{doc.descripcion}</Text>}

              <View style={styles.documentActions}>
                <TouchableOpacity style={styles.viewButton} onPress={() => viewDocument(doc)}>
                  <Text style={styles.viewButtonText}>Ver PDF</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.deleteButton} onPress={() => deleteDocument(doc.id)}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalForm}>
              ...
              <View style={styles.tipoContainer}>
                {['consulta', 'vacuna', 'examen', 'receta', 'otro'].map((tipo) => (
                  <TouchableOpacity key={tipo} style={[styles.tipoOption, newDoc.tipo === tipo && styles.selectedTipoOption]} onPress={() => setNewDoc({ ...newDoc, tipo })}>
              ...
              <TextInput style={styles.input} placeholder="Ej: Consulta general" value={newDoc.titulo} onChangeText={(text) => setNewDoc({ ...newDoc, titulo: text })} />
              ...
              <TouchableOpacity style={styles.pickButton} onPress={pickDocument}>
                <Text style={styles.pickButtonText}>{newDoc.file ? `✓ ${newDoc.file.name}` : '📎 Seleccionar PDF'}</Text>
              </TouchableOpacity>
            </ScrollView>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity style={[styles.modalButton, styles.saveButton]} onPress={uploadDocument} disabled={uploading}>
                {uploading ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveButtonText}>Subir</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={showPdfModal} animationType="slide" transparent={false} onRequestClose={() => setShowPdfModal(false)}>
        <SafeAreaView style={styles.pdfModalContainer}>
          <View style={styles.pdfHeader}>
            <TouchableOpacity style={styles.closeButton} onPress={() => setShowPdfModal(false)}>
