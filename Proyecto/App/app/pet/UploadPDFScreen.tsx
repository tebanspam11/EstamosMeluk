...
export default function UploadPDFScreen({ navigation, route }: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Input oculto */}
      <input type="file" accept="application/pdf" ref={fileInputRef} style={{ display: 'none' }} onChange={handleFileChangeWeb} />

      {/* Botón de selección */}
      <TouchableOpacity style={styles.selectBtn} onPress={() => fileInputRef.current?.click()}>
        <Text style={styles.selectText}>{selectedFile ? 'Cambiar archivo' : 'Seleccionar PDF'}</Text>
      </TouchableOpacity>

      {selectedFile && <Text style={{ marginBottom: 10 }}>📄 {selectedFile.name}</Text>}

      {/* Botón de subida */}
      <TouchableOpacity style={[styles.uploadBtn, (!selectedFile || uploading) && { backgroundColor: '#ccc' }]} disabled={!selectedFile || uploading} onPress={handleUpload}>
