...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, ScrollView, Image, Alert, Modal, FlatList, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function EditPetProfileScreen() {
  ...
  const cargarDatosMascota = async () => {
    ...
    if (petFromRoute) {
      const fechaStr = petFromRoute.fecha_nacimiento ? new Date(petFromRoute.fecha_nacimiento).toISOString().split('T')[0] : '';
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <TouchableOpacity style={styles.imageContainer} onPress={pickImage}>
              {petImage ? (
                ...
                <View style={styles.imagePlaceholder}>
                  <Text style={styles.imagePlaceholderIcon}>{especie ? getPetIcon(especie) : '🐾'}</Text>
          ...
          <View style={styles.form}>
            ...
            <TouchableOpacity style={styles.dropdownButton} onPress={() => setShowSpeciesModal(true)}>
              <Text style={especie ? styles.dropdownTextSelected : styles.dropdownTextPlaceholder}>{especie ? `${getPetIcon(especie)} ${especie}` : 'Selecciona una especie'}</Text>
            </TouchableOpacity>
            ...
            {/* Botón de Guardar */}
            <TouchableOpacity style={[styles.saveButtonLarge, (!isFormValid() || saving) && styles.saveButtonDisabled]} onPress={handleSave} disabled={!isFormValid() || saving}>
              <Text style={styles.saveButtonLargeText}>{saving ? 'Guardando...' : 'Guardar Cambios'}</Text>
            </TouchableOpacity>

            {/* Botón Cancelar */}
            <TouchableOpacity style={styles.cancelButtonLarge} onPress={() => navigation.goBack()}>
